...
import edu.usf.cutr.go_sync.gui.StatisticsDisplay;
...
import edu.usf.cutr.go_sync.tools.parser.DeleteNodeParser;
import edu.usf.cutr.go_sync.tools.parser.NodeHistoryParser;
import edu.usf.cutr.go_sync.tools.parser.OsmPrimitiveHistoryParser;
...
//            InputSource inputSource = new InputSource("DataFromServerRELATION.osm");
    ...
    public void getPastHistoryOfDeletedNode(String cs, HashSet<String> dnode) {
        String urlSuffix = "changeset/"+cs+"/download";
        String url = SERVER_URL + urlSuffix;
        try {
            // get data from server
            ...
            // get data from file - need to remove this for REAL APPLICATION
//            InputSource inputSource = new InputSource("DataFromServerRELATION.osm");
            DeleteNodeParser par = new DeleteNodeParser(dnode);
            SAXParserFactory.newInstance().newSAXParser().parse(inputSource, par);

            Hashtable<String, String> deletedNode = new Hashtable<String, String>();
            deletedNode.putAll(par.getDeletedNodes());

            ArrayList<String> keys = new ArrayList<String>();
            keys.addAll(deletedNode.keySet());
            for(int i=0; i<keys.size(); i++){
                System.out.println(getNodeHistory(deletedNode.get(keys.get(i))));
            }
        } catch(IOException e) {
            System.out.println(e);
        } catch(SAXException e) {
...
//            InputSource inputSource = new InputSource("DataFromServerRELATION.osm");
     ...
     *
     */
    private String getNodeHistory(String osmid){
        Stop st=null;
        String urlSuffix = "node/"+osmid+"/history";
        String url = SERVER_URL + urlSuffix;
        System.out.println("Retrieving node "+osmid+" ...");
        try {
            // get data from server
            String s = sendRequest(url, "GET", "");
            InputSource inputSource = new InputSource(new StringReader(s));
            // get data from file - need to remove this for REAL APPLICATION
//            InputSource inputSource = new InputSource("DataFromServerRELATION.osm");
            NodeHistoryParser par = new NodeHistoryParser();
            SAXParserFactory.newInstance().newSAXParser().parse(inputSource, par);
            return par.getLatestUser();
        } catch(IOException e) {
            System.out.println(e);
        } catch(SAXException e) {
            System.out.println(e);
    ...
    public String getAllUsersOfNode(String osmid){
        String urlSuffix = "node/"+osmid+"/history";
        String url = SERVER_URL + urlSuffix;
        System.out.println("Retrieving history of node "+osmid+"...");
        String history="";
        try {
            // get data from server
            String s = sendRequest(url, "GET", "");
        ...
        while (true) {
            try {
                ...
                try {
                    ...
                    if(taskOutput!=null) taskOutput.append("Connecting "+url+" using method "+method+" "+retry+"\n");
